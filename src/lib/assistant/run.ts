...
const DESK_MAP =
  "Whole desk, not only sector analysis. US dashboard / · US pattern desk /desk · US Top 50 /stocks · NSE pattern desk /nse · Nifty 50 /nse/stocks · Market environment /environment · Portfolio /portfolio · Time frame /timeframe · Sector analysis /sector · IPO /ipo · Algorithm /algorithm. Ask any of those.";
...
function navNote(q: string) {
  ...
  if (/\bipo\b/.test(q) && /where|open|tab/.test(q)) return "IPO is in the top header. Open, upcoming, and recent issues from Chittorgarh.";
  if (/\balgorithm\b/.test(q) && /where|open|tab/.test(q)) {
    return "Algorithm is in the top header. Book posterior uses the portfolio book; the Nifty 50 spread engine is the model and is not live yet.";
  }
  return null;
}
...
const SYSTEM = `You are the Eminent Corpus desk assistant for the whole app: US/NSE desks, Nifty 50 / Top 50, market environment, portfolio blotter, time-frame journal, sector analysis, IPO, and Algorithm. Answer only from the snapshot JSON. Be concise. Never invent prices or P&L. End with "Not a recommendation" when you discuss holdings.`;
